...
public enum StatesEnum {
    ALABAMA("Alabama", "AL"), ALASKA("Alaska", "AK"), AMERICAN_SAMOA("American Samoa", "AS"), ARIZONA("Arizona", "AZ"), ARKANSAS(
            ...
            "Unknown", "");
    private static final Map<String, StatesEnum> STATES_BY_ABBR = new HashMap<>();

    /* static initializer */
    static {
        for (StatesEnum state : values()) {
            STATES_BY_ABBR.put(state.getAbr(), state);
        }
    }
    ...
    StatesEnum(String name, String abr) {
        this.name = name;
        this.abr = abr;
    }

    public static StatesEnum valueOfAbbreviation(final String abbr) {
        final StatesEnum state = STATES_BY_ABBR.get(abbr);
        if (state != null) {
            return state;
        } else {
            return UNKNOWN;
